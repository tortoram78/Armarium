// Recommendation + gap analysis — pure reasoning over the facet space (Layer 2). A trip is reduced to a
// set of required capabilities with severities; each is evaluated across the inventory into
// satisfied / uncertain (blocked_unknown ⇒ "verify") / gap. Picks are the items that satisfy something;
// gaps are required capabilities nothing satisfies.

import type { ResolvedItem } from "../resolved";
import { type CapabilityKey, type CapResult, evaluateCapability } from "../capabilities";
...
export function recommend(items: ResolvedItem[], env: TripEnvelope): RecommendationResult {
  const outcomes: CapabilityOutcome[] = env.required.map(({ capability, severity }) => {
    const satisfiedBy: ItemRef[] = [];
    const blockedBy: ItemRef[] = [];
    for (const it of items) {
      const r: CapResult = evaluateCapability(it, capability);
    ...
    return { capability, severity, status, satisfiedBy, blockedBy };
  });
  ...
  const gaps: Gap[] = outcomes
    ...
    .map((o) => ({ capability: o.capability, severity: o.severity }));
